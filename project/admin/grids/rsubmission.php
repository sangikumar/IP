...
$grid->SelectCommand = 'SELECT id, candidateid, employeeid, submissiondate, type, name, email, phone, url, notes FROM mkt_submission where employeeid = "'. $employeeid .'" or employeeid in (select id from employee where mgrid = "'. $employeeid .'")';
...
$grid->setColModel();
$grid->setUrl('../grids/rsubmission.php?employeeid='.$employeeid);
...
$grid->setSelect("candidateid", "SELECT distinct candidateid as id, name as name FROM candidate where status in ('Marketing', 'Placed', 'Placed', 'OnProject-Mkt') order by name");
...
$grid->setSelect("employeeid", "SELECT distinct id, name FROM employee where id = $employeeid or mgrid = $employeeid  order by name");
...
$grid->setNavOptions('view',array("width"=>750, "dataheight"=>400, "viewCaption"=>"Submission List"));
