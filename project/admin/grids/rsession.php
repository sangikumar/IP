...
$grid->SelectCommand = 'select sessionid, instructorid, candidateid, title, status, sessiondate, type, candidate2id, candidate3id, candidate4id, candidate5id, subject, performance, feedback, recorded notes from session where instructorid = "'. $userid .'"';
$grid->table = 'session';
...
$grid->setColModel();
$grid->setUrl('../grids/rsession.php?userid='.$userid);
...
$grid->setSelect("candidateid", "select '' as id, ' Group Mock' as name from dual union SELECT distinct candidateid as id, name as name FROM candidate where status in ('Active', 'Mocks', 'Marketing', 'OnProject-Mkt') order by name");
...
$grid->setSelect("candidate2id", "select null as id, '' as name from dual union SELECT distinct candidateid as id, name as name FROM candidate where status in ('Active', 'Mocks', 'Marketing', 'OnProject-Mkt') order by name");
...
$grid->setSelect("candidate3id", "select null as id, '' as name from dual union SELECT distinct candidateid as id, name as name FROM candidate where status in ('Active', 'Mocks', 'Marketing', 'OnProject-Mkt') order by name");
...
$grid->setSelect("candidate4id", "select null as id, '' as name from dual union SELECT distinct candidateid as id, name as name FROM candidate where status in ('Active', 'Mocks', 'Marketing', 'OnProject-Mkt') order by name");
...
$grid->setSelect("candidate5id", "select null as id, '' as name from dual union SELECT distinct candidateid as id, name as name FROM candidate where status in ('Active', 'Mocks', 'Marketing', 'OnProject-Mkt') order by name");
